...
- Standalone `texture_atlas` example demonstrating `DynamicCollider` updates when atlas frames change (defaults to `rapier2d`; `avian2d` via `-F avian2d`).

### Changed

- Use in-crate `edges` binary types (`BinaryImage`, `BinaryView`, `BinaryImageView`) instead of the removed `edges::binary_image` re-export.
- Builder is bounded on `BinaryImageView` rather than `GenericImageView<Pixel = Bit>`.
- Bevy conversion errors come from `edges::IntoBinaryImageError`.
- Moved the texture-atlas demo out of the main collider examples into `texture_atlas`.
